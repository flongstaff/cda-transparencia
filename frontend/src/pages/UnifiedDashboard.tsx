/**
 * Unified Dashboard - Municipal Transparency Portal
 * Clean, single-source approach to transparency data
 */
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8 text-center">
          ...
          <p className="text-sm text-gray-500 mt-2">
            Cargando {metrics.availableYears.length} años de datos financieros y de auditoría
          </p>
        </div>
      </div>
    );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Datos completos de {selectedYear} - {metrics.totalDocuments} documentos procesados
        </p>
        <div className="mt-2 flex items-center space-x-4 text-sm text-gray-500">
          <span>📊 {metrics.categories.length} categorías</span>
          <span>🔗 {metrics.availableYears.length} años disponibles</span>
          <span>✅ {formatPercentage(metrics.auditCompletionRate)} auditoría completada</span>
        </div>
      </div>
        ...
        <div className="border-b border-gray-200 p-6">
          <div className="flex items-center justify-between">
            ...
            <div className={`px-3 py-1 rounded-full text-sm font-medium ${
              metrics.auditCompletionRate > 80 ? 'bg-green-100 text-green-800' :
              metrics.auditCompletionRate > 50 ? 'bg-yellow-100 text-yellow-800' :
              'bg-red-100 text-red-800'
            }`}>
              {metrics.auditCompletionRate > 80 ? '✅ Sistema Completo' :
               metrics.auditCompletionRate > 50 ? '⚠️ En Desarrollo' :
               '❌ Datos Limitados'}
            </div>
          </div>
          <p className="text-gray-600 mt-1">
            Sistema completo de transparencia con {metrics.totalDocuments} documentos procesados
          </p>
        </div>
        <div className="p-6">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-600">{metrics.totalDocuments}</p>
                </div>
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">{metrics.availableYears.length}</p>
                </div>
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-purple-600">{metrics.categories.length}</p>
                </div>
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-orange-600">{metrics.availableYears.length}</p>
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
          ...
          <div className="flex items-center">
            <span className="text-sm font-medium text-green-600">
              {formatPercentage(metrics.executionRate)}
            </span>
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
          ...
          <p className="text-3xl font-bold text-gray-900 mb-2">
            {formatPercentage(metrics.auditCompletionRate)}
          </p>
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div
              className="bg-purple-600 h-2 rounded-full"
              style={{ width: `${metrics.auditCompletionRate}%` }}
            ></div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
          ...
          <p className="text-3xl font-bold text-gray-900 mb-2">
            {currentYearData?.contracts?.length || 0}
          </p>
          <p className="text-sm text-gray-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          ...
          <p className="text-sm text-orange-600 mt-2">
            {currentYearData?.contracts?.length || 0} contratos registrados
          </p>
        </Link>
        ...
        >
          ...
          <p className="text-sm text-green-600 mt-2">
            {metrics.totalDocuments} documentos disponibles
          </p>
        </Link>
      </div>
    </div>
