/**
 * Investment Report Chart Component
 * Displays investment report data
 */
...
// Props for the InvestmentReport Chart component
interface InvestmentReportChartProps {
  height?: number;
  width?: number | string;
...
const InvestmentReportChart: React.FC<InvestmentReportChartProps> = ({
  height = 400,
  width = '100%',
  chartType = 'line',
  showTitle = true,
  ...
  const { data, isLoading, isError, error: queryError } = useQuery({
    queryKey: ['chart-data', 'Investment_Report', year],
    queryFn: () => chartDataService.loadChartData('Investment_Report'),
  ...
  const handleDataPointClick = (dataPoint: any) => {
    console.log('Investment Report data point clicked:', dataPoint);
    ...
    return (
      <Box display="flex" justifyContent="center" alignItems="center" height={height} className={className}>
        ...
        <Typography variant="body1" sx={{ ml: 2 }}>
          Cargando datos del Informe de Inversiones...
        </Typography>
      </Box>
    );
  }
  ...
  if (error) {
    return (
      <Alert severity="error" className={className}>
        Error cargando datos del Informe de Inversiones: {error}
      </Alert>
    );
  }
  ...
  if (!chartData || chartData.length === 0) {
    return (
      <Alert severity="warning" className={className}>
        No hay datos disponibles del Informe de Inversiones
      </Alert>
    );
  }
  ...
  return (
    <BaseChart
      ...
      xAxisKey="year"
      yAxisKeys={yAxisKeys}
      title={showTitle ? CHART_TYPE_NAMES.Investment_Report : undefined}
      description={showDescription ? CHART_TYPE_DESCRIPTIONS.Investment_Report : undefined}
      height={height}
      width={width}
      className={className}
...
export default InvestmentReportChart;
