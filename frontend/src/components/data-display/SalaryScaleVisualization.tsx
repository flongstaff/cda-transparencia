...
const SalaryScaleVisualization: React.FC = () => {
  ...
  const formatLocalCurrency = (value: number): _string => {
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center">
            ...
            <span className="font-medium text-gray-900">
              Valor del Módulo: {formatLocalCurrency(salaryData.moduleValue)}
            </span>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {formatLocalCurrency(salaryData.totalPayroll)}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-6 border-l-4 border-purple-500">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {formatLocalCurrency(salaryData.averageSalary)}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip 
                  formatter={(value) => [formatLocalCurrency(Number(value)), '']}
                />
                <Legend />
              </PieChart>
            </ResponsiveContainer>
              ...
              {salaryData.positions.map((position, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    {formatLocalCurrency(position.grossSalary)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {formatLocalCurrency(position.somaDeduction)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    {formatLocalCurrency(position.ipsDeduction)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    {formatLocalCurrency(position.netSalary)}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm">
            ...
            <ul className="text-sm text-gray-600 space-y-1">
              <li>• Más alto: {formatLocalCurrency(Math.max(...salaryData.positions.map(p => p.grossSalary)))}</li>
              <li>• Más bajo: {formatLocalCurrency(Math.min(...salaryData.positions.map(p => p.grossSalary)))}</li>
              <li>• Promedio: {formatLocalCurrency(salaryData.averageSalary)}</li>
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm">
            ...
            <ul className="text-sm text-gray-600 space-y-1">
              ...
              <li>• Valor del módulo: {formatLocalCurrency(salaryData.moduleValue)}</li>
              <li>• Nómina total: {formatLocalCurrency(salaryData.totalPayroll)}</li>
