...
const ContractsTracker: React.FC = () => {
  ...
  const [yearlyFlow, setYearlyFlow] = useState<MoneyFlowData[]>([]);
  ...
  useEffect(() => {
    const loadContractsData = async () => {
      try {
        ...
        // Load data using comprehensive service
        const comprehensiveData = await comprehensiveDataService.loadComprehensiveData();

        // Extract contract data from comprehensive structure
        const contractData: ContractData[] = [];
        Object.values(comprehensiveData.structured.contracts).forEach(contracts => {
          if (Array.isArray(contracts)) {
            contractData.push(...contracts);
          }
        });

        // Create mock yearly flow for now
        setYearlyFlow([]);
        setContracts(contractData);
        console.log('Contracts data loaded:', { contractData, comprehensiveData }); // Debug: Check console for all numbers
      } catch (error) {
        console.error('Error loading contracts data:', error);
        setError('Error al cargar los datos de contratos y ejecución financiera');
      } finally {
        setLoading(false);
      }
    };
