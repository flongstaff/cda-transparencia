  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white p-6 rounded-lg">
        <h1 className="text-3xl font-bold">Transparencia Municipal - {data.municipality}</h1>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
          <div>
            <h3 className="text-lg font-semibold mb-4">Resumen Presupuestario</h3>
              ...
              {budget_overview.budget_by_area.map((area, index) => (
                <div key={index} className="flex justify-between items-center p-2 hover:bg-gray-50 rounded">
                  ...
                  <div className="text-right">
                    <div className="font-semibold">{area.percentage}%</div>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
          <div className="p-4 bg-green-50 rounded-lg border border-green-200">
            <div className="text-2xl font-bold text-green-700">{transparency_indicators.budget_accessibility}%</div>
            <div className="text-sm text-green-600">Accesibilidad Presupuestaria</div>
          </div>
          
          <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
            <div className="text-2xl font-bold text-blue-700">{transparency_indicators.document_availability}%</div>
            <div className="text-sm text-blue-600">Disponibilidad de Documentos</div>
          </div>
          
          <div className="p-4 bg-purple-50 rounded-lg border border-purple-200">
            <div className="text-2xl font-bold text-purple-700">{transparency_indicators.public_engagement}%</div>
            <div className="text-sm text-purple-600">Participación Ciudadana</div>
          </div>
          
          <div className="p-4 bg-teal-50 rounded-lg border border-teal-200">
            <div className="text-2xl font-bold text-teal-700">{transparency_indicators.info_quality}%</div>
            <div className="text-sm text-teal-600">Calidad de la Información</div>
          </div>
        </div>
      </StandardizedCard>
      ...
      >
        {recent_licitaciones && recent_licitaciones.length > 0 ? (
          ...
          <div className="text-center py-8 text-gray-500 mt-4">
            ...
            <p className="mt-2">No hay licitaciones recientes disponibles</p>
          </div>
        )}
      </StandardizedCard>
      ...
      >
        {data.documents && data.documents.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            {data.documents.map((doc) => (
              <div key={doc.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                ...
                <p className="text-sm text-gray-500 mt-1 capitalize">{doc.category}</p>
          ...
          <div className="text-center py-8 text-gray-500 mt-4">
            <FileText className="mx-auto h-12 w-12 text-gray-400" />
            <p className="mt-2">No hay documentos disponibles</p>
          </div>
        )}
      </StandardizedCard>
    </div>
