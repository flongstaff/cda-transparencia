...
class OSINTComplianceMonitor:
    ...
    def check_activity(self, activity: Dict) -> Tuple[ComplianceStatus, List[str]]:
        ...
        # Convert any bytes objects to string representation for JSON serialization
        serializable_activity = activity.copy()
        for k, v in serializable_activity.items():
            if isinstance(v, bytes):
                try:
                    serializable_activity[k] = v.decode('utf-8')
                except UnicodeDecodeError:
                    serializable_activity[k] = str(v) # Fallback to string representation

        activity['id'] = hashlib.md5(
            json.dumps(serializable_activity, sort_keys=True).encode()
        ).hexdigest()[:8]
    ...
    def _check_no_pii(self, activity: Dict) -> Tuple[bool, Optional[str]]:
        ...
        if not isinstance(data, str):
            # If data is not a string, try to decode it. If it fails, it's likely binary.
            try:
                data = data.decode('utf-8')
            except (UnicodeDecodeError, AttributeError): # AttributeError for non-bytes objects
                # If decoding fails or it's not bytes, check if it's a PDF
                if isinstance(data, bytes) and data.startswith(b'%PDF'):
                    return True, None # Skip PII check for PDF binary content
                # For other non-string, non-PDF binary data, skip PII check
                return True, None
    ...
    def _check_no_exploits(self, activity: Dict) -> Tuple[bool, Optional[str]]:
        ...
        if not isinstance(data, str):
            ...
            try:
                ...
                # If decoding fails or it's not bytes, check if it's a PDF
                if isinstance(data, bytes) and data.startswith(b'%PDF'):
                    return True, None # Skip exploit check for PDF binary content
                # For other non-string, non-PDF binary data, skip exploit check
                return True, None
...
def integrate_with_osint_tool(monitor: OSINTComplianceMonitor):
    ...
    def compliant_web_request(url: str, **kwargs) -> Optional[requests.Response]:
        ...
        try:
            response = requests.get(url, **{k: v for k, v in kwargs.items() if k not in ['purpose', 'robots_checked', 'robots_allowed']})
            ...
            try:
                ...
                try:
                    ...
                    decoded_content = response.content.decode('utf-8', errors='ignore') # Fallback with error ignoring
                    logger.warning(f"Decoding error for {url}, used utf-8 with error ignoring.")

            # Check response for PII
            response_activity = {
                'type': 'response_check',
                'data': decoded_content[:1000] if decoded_content else '',  # Check first 1000 chars of decoded content
                'url': url
            }
