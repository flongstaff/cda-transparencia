...
async def _get_links(
    ...
    # Recurse if depth > 1
    if depth > 1:
        # Find subpages to recurse into
        subpage_pattern = re.compile(r'/$')
        subpages = []
        for link in soup.find_all('a', href=True):
            href = link['href']
        ...
        for subpage in subpages:
            try:
                sub_results = await _get_links(subpage, pattern, depth=depth-1, session=session)
                for result in sub_results:
                    if result not in seen:
                        seen.add(result)
                        results.append(result)
            except Exception as e:
                print(f"Error scraping subpage {subpage}: {e}")
