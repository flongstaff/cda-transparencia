...
class DataDeployment:
    ...
    def copy_data_to_cloudflare_pages_location(self):
        """Copy data to locations accessible by Cloudflare Pages"""
        logger.info("Copying data to Cloudflare Pages accessible locations...")
        
        # Create data directory in Cloudflare deployment folder
        cloudflare_data_dir = self.base_dir / "cloudflare-deploy" / "public" / "data"
        cloudflare_data_dir.mkdir(parents=True, exist_ok=True)
        
        # Create subdirectories
        (cloudflare_data_dir / "csvs").mkdir(parents=True, exist_ok=True)
        (cloudflare_data_dir / "pdfs").mkdir(parents=True, exist_ok=True)
        
        # Copy CSV files to Cloudflare location
        processed_csv_dir = self.processed_data_dir / "csvs"
        if processed_csv_dir.exists():
            for csv_file in processed_csv_dir.glob("*.csv"):
                dest_path = cloudflare_data_dir / "csvs" / csv_file.name
                shutil.copy2(csv_file, dest_path)
                logger.info(f"Copied {csv_file.name} to Cloudflare location: {dest_path}")
        
        # Copy PDF files to Cloudflare location
        processed_pdf_dir = self.processed_data_dir / "pdfs"
        if processed_pdf_dir.exists():
            for pdf_file in processed_pdf_dir.glob("*.pdf"):
                dest_path = cloudflare_data_dir / "pdfs" / pdf_file.name
                shutil.copy2(pdf_file, dest_path)
                logger.info(f"Copied {pdf_file.name} to Cloudflare location: {dest_path}")
    ...
    def deploy_for_github_pages(self):
        ...
        # Copy data to Cloudflare Pages accessible locations
        self.copy_data_to_cloudflare_pages_location()
